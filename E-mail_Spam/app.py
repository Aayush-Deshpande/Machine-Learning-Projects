...
st.markdown("Enter the content of an email below to instantly check if it is **Spam** or **Not Spam (Ham)**.")
...
email_input = st.text_area("Email Content", height=200, placeholder="Paste your email text here...")
...
if st.button("Classify Email", type="primary"):
    if not email_input.strip():
        ...
        with st.spinner("Analyzing text..."):
            transformed_sms = transform_text(email_input)
            ...
            st.divider()
            if result == 1:
                st.error("🚨 **Prediction:** This email is classified as **SPAM**.")
            else:
                st.success("✅ **Prediction:** This email is classified as **NOT SPAM** (Ham).")
